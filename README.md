...
Pick whichever roadmap item fits your needs best:

1. **"Fast" build profile** – add presets that fix `--level 5`, 2 MiB frames, and aggressive multi-threading for quick
   archival passes on slower PCs.
2. **Full GUI front-end** – extend the existing Win32 UI with drag-and-drop, multi-job queueing, and preset buttons.
3. **Library embedding** – expose the compressor/decompressor as a static library so other launchers (FBI, GodMode9,
   etc.) can ship a single `z3ds` module without replicating logic.

## Testing in a private repository

If you want to evaluate these changes privately before opening a public fork, the easiest workflow is to add a second
remote that points to your private repository and push the current branch there:

```bash
git remote add private git@github.com:your-user/your-private-repo.git
git push private HEAD:main   # or whatever branch name you prefer
```

You can then pull or clone that private repository on another machine, build the CLI/GUI, and share binaries with your
team without exposing the code publicly. When you're happy with the results, push the same branch to your public fork or
open a PR upstream.

Let me know which one you want prioritized and I can scope it next.
